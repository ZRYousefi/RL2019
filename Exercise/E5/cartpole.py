...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000,
    ...
    policy = Policy(observation_space_dim, action_space_dim, std_mode=std_mode)
    agent = Agent(policy)
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            action, action_probabilities = agent.get_action(observation, timesteps)
        ...
        agent.episode_finished(episode_number, baseline_mode=baseline_mode)
...
def test(env_name, episodes, params, render, std_mode):
    # Create a Gym environment
    env = gym.make(env_name)
...
if __name__ == "__main__":
    ...
    if args.test is None:
        try:
            train(args.env, train_episodes=args.train_episodes, baseline_mode=baseline_mode,
                std_mode=std_mode)
        ...
        print("Testing...")
        test(args.env, 100, state_dict, args.render_test, std_mode=std_mode)
