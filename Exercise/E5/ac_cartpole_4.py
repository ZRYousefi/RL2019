...
from ac_agent_4 import Agent, Policy
...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000):
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            action, action_probs, critic_value = agent.get_action(observation)
            previous_observation = observation

            # Perform the action on the environment, get new state and reward
            observation, reward, done, info = env.step(action.detach().numpy())
            # Calculate TD error
            agent.each_timestep(observation, reward, done, critic_value)
            # Store action's outcome (so that the agent can improve its policy)
            agent.store_outcome(previous_observation, action_probs, reward)
