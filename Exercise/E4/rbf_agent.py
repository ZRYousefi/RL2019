...
class Agent(object):
    ...
    def update_estimator(self):
        ...
        next_qs = np.max([q.predict(featurized_next_states) for q in self.q_functions], axis=0)
        ...
        #         targets[i] = rewards[i]
        #     else:
        #         targets[i] = [rewards[i] + self.gamma * next_qs]
        targets = self.gamma * next_qs + rewards
